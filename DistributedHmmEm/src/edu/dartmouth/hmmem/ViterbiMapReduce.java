...
public class ViterbiMapReduce {
	...
	public static class ViterbiMapper extends MapReduceBase implements
		...
		public void map(LongWritable key, Text value,
				OutputCollector<NullWritable, TaggedObservationSequence> output, Reporter reporter)
				throws IOException {
			if (failure) {
		...
		private static TaggedObservationSequence calculateViterbiTagging(List<String> observationSequence, Map<StringPair, Double> transLogProbDict,
			...
			System.out.println("Optimal tagging:");
			System.out.println(optimalTagging);
			
			return optimalTagging;
		}
	}
	
	public static class ViterbiReducer extends MapReduceBase
			implements Reducer<NullWritable, TaggedObservationSequence, NullWritable, TaggedObservationSequence> {

		/**
		 * Nothing special, just output the viterbi tagging values.
		 */
		@Override
		public void reduce(NullWritable key,
				Iterator<TaggedObservationSequence> viterbiTaggings,
				OutputCollector<NullWritable, TaggedObservationSequence> output,
				Reporter reporter) throws IOException {
			while (viterbiTaggings.hasNext()) {
				TaggedObservationSequence viterbiTagging = viterbiTaggings.next();
				output.collect(NullWritable.get(), viterbiTagging);
			}
		}		
	}
